...
mod tests {
    ...
    fn test_sse_open_frame_roundtrip() {
        ...
        let frame = TunnelFrame::sse_open("req-3", "/events", Some(headers.clone()));
        let json = serde_json::to_string(&frame).unwrap();
        let parsed: TunnelFrame = serde_json::from_str(&json).unwrap();
    ...
    fn test_sse_event_frame_roundtrip() {
        let frame = TunnelFrame::sse_event("req-4", "data: hello\n\n");
        let json = serde_json::to_string(&frame).unwrap();
        let parsed: TunnelFrame = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.frame_type, "sse_event");
        assert_eq!(parsed.request_id.as_deref(), Some("req-4"));
    ...
    fn test_sse_close_frame_roundtrip() {
        let frame = TunnelFrame::sse_close("req-5");
        let json = serde_json::to_string(&frame).unwrap();
        let parsed: TunnelFrame = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.frame_type, "sse_close");
        assert_eq!(parsed.request_id.as_deref(), Some("req-5"));
        assert!(parsed.method.is_none());
        assert!(parsed.path.is_none());
        assert!(parsed.headers.is_none());
        assert!(parsed.body.is_none());
