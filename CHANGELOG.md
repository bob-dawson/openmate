...
- **会话内实时更新失效**：修复依赖注入作用域问题——SSE 通知处理器与「当前活动会话」被注入成两个不同实例，导致会话内的 SSE 通知被全部跳过，只能依赖 15 秒轮询。修复后：工具调用一开始就出现（运行中状态），自己发送的消息回显更快
...
- **国内下载加速**：App 与 Bridge 的升级下载新增国内镜像（AtomGit），并按**系统区域**自动选择下载源——中国大陆优先国内镜像、GitHub 兜底；其他地区优先 GitHub、国内镜像兜底。镜像失败会自动切换到下一个；旧版本客户端仍可正常解析版本信息并升级
...
- **技能名称显示**：修复调用技能（skill）时只显示 `skill`、未显示技能名称的问题
...
- **升级后启动崩溃**：修复数据库 schema 变更但未递增版本号导致 Room 完整性校验失败、App 启动崩溃的问题（DB v7→v8，升级时重建本地缓存）
...
- **管理页面国际化**：Bridge 管理页面支持中英文自动切换，根据浏览器语言环境自动选择
- **License 内嵌显示**：设置页面的 License 改为内嵌对话框显示，不再打开外部浏览器
- **Debug 签名统一**：Debug 构建使用与 Release 相同的签名，测试时可直接覆盖安装

### 问题修复

- **opencode 重启**：修复 Windows 上重启 opencode 时被强制终止导致端口占用无法恢复的问题，改为通过 console 信号优雅退出
- **Bridge 重启不中断 opencode**：Bridge 重启时检测到 opencode 已在运行则直接接管，不再无意义重启
- **Bridge 版本检查**：修复设置页面 Bridge 版本检查依赖异步未完成数据导致错误显示"可升级"的问题
- **用户消息渲染**：修复用户消息换行丢失和 Markdown 格式不渲染的问题
- **断开重连崩溃**：修复断开连接后立即重新进入实例导致 No active database 崩溃的问题
