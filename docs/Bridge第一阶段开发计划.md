...
Bridge 监听一个 HTTP 端口（如 `:4097`），接收客户端请求，转发到本地 opencode（如 `:4096`）。

**REST 代理**：
- 客户端发到 Bridge 的 `/api/opencode/*` 请求，Bridge 去掉前缀后转发到 opencode
- 例：`GET http://bridge:4097/api/opencode/session` → `GET http://localhost:4096/session`
- 透传请求头、请求体、响应状态码、响应头
...
- 按需启动 opencode（`opencode serve --hostname 127.0.0.1 --port 4096`）
...
**当前**：`ServerProfile.address:port` → 直连 opencode（`http://address:4096`）
...
4. **opencode 端口冲突**：Bridge 和 opencode 各占一个端口，确保不冲突。Bridge 默认 4097，opencode 默认 4096
