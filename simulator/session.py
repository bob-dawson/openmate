...
Creates a new session and sends a message, verifying AI response.
...
Usage:
    python session.py                                    # Default: title "Test Session", message "Hello, what is 2+2?"
    python session.py --title "My Session" --message "Explain recursion"
    python session.py --wait 30                          # Wait longer for AI response

State guard: Script verifies it starts from the instance detail page (with "New Session" button).
...
def create_session(d, title):
    ...
    print("=== Step 2: Fill title and create ===")
...
def session(d, instance_name, title, message, wait_seconds):
    if not navigate_to_instance_detail(d, instance_name):
        return False

    if not create_session(d, title):
        return False

    if not send_message(d, message, wait_seconds):
        return False

    print(f"\nSession complete! Title: {title!r}, Message: {message!r}")
    return True
...
def main():
    ...
    success = session(d, args.instance, args.title, args.message, args.wait)
